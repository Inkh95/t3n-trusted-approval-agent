...
const rules:Record<string,PolicyDecision>={
 ...
 "task.claim":{decision:"approval_required",risk:"high",reason:"A claim needs standing approval evidence.",rule:"task.claim"},
...
export class PolicyEngine{evaluate(a:AgentAction):PolicyDecision{if(a.type==="payment.send"){
 if(a.currency!=="EUR"||typeof a.amount!=="number"||!Number.isFinite(a.amount)||a.amount<=0)return{decision:"deny",risk:"critical",reason:"Payment requires a positive finite EUR amount.",rule:"payment.send.invalid-amount"};
 if(a.amount>50)return{decision:"deny",risk:"critical",reason:"Amount exceeds the configured 50 EUR limit.",rule:"payment.send.limit"};
}if(a.type==="task.claim"&&a.payload.standingApproval===true)return{decision:"allow",risk:"low",reason:"All standing approval controls were verified before claim.",rule:"task.claim.standing-approval"};return rules[a.type]??{decision:"approval_required",risk:"high",reason:"Unknown actions fail closed and require review.",rule:"default-deny-by-review"}}}
