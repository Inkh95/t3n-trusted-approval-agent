...
export class PolicyEngine{evaluate(a:AgentAction):PolicyDecision{if(a.type==="payment.send"&&(a.amount??0)>50)return{decision:"deny",risk:"critical",reason:"Amount exceeds the configured 50 EUR limit.",rule:a.type};if(a.type==="task.claim"&&a.payload.standingApproval===true)return{decision:"allow",risk:"low",reason:"All standing approval controls were verified before claim.",rule:"task.claim.standing-approval"};return rules[a.type]??{decision:"approval_required",risk:"high",reason:"Unknown actions fail closed and require review.",rule:"default-deny-by-review"}}}
